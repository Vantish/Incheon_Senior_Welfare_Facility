...
본 리포지토리는 인천광역시 내 노인복지시설 데이터를 기반으로,  
사용자의 지리정보 기반 입력값을 활용하여 최적화된 노인복지시설을 지리적 근접성 및 컨텍스트 정보에 따라 추천하며,  
주변 맛집, 여가·체육시설, 대중교통 정류장 등 다양한 관심 지점(Point of Interest, POI)을 통합 제공하는  
Streamlit 기반 위치기반 서비스(Location-Based Service, LBS) 애플리케이션입니다.
